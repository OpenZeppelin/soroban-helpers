...
impl Provider {
    ...
    pub async fn simulate_transaction(&self, tx_envelope: &TransactionEnvelope) -> Result<stellar_rpc_client::SimulateTransactionResponse, Error> {
        self.rpc_client.simulate_transaction_envelope(tx_envelope).await
    }
    
    pub async fn send_transaction(&self, tx_envelope: &TransactionEnvelope) -> Result<GetTransactionResponse, Error> {
